...
function extractMessageText(content) {
  ...
  if (Array.isArray(content.parts)) {
    return sanitizeString(content.parts.map((part) => typeof part === 'string' ? part : (part && typeof part.text === 'string' ? part.text : '')).filter(Boolean).join(' '), 1200);
  }
  return '';
}
function scanForSourceItems(value, acc = []) {
  if (!value || typeof value !== 'object') return acc;
  if (Array.isArray(value)) { value.forEach((item) => scanForSourceItems(item, acc)); return acc; }
  ...
  Object.values(value).forEach((child) => { if (child && typeof child === 'object') scanForSourceItems(child, acc); });
  return acc;
}
...
function buildConversationTurns(raw) {
  ...
  orderedNodes.forEach((node) => {
    ...
    if (role === 'user') {
      ...
      if (prompt) {
        currentTurn = { index: turns.length + 1, prompt, queries: [], sourceItems: [] };
...
function parseChatgptPayload(raw, conversationId, pageUrl) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw) || !raw.mapping || typeof raw.mapping !== 'object' || Array.isArray(raw.mapping)) return null;
  const queries = [];
  const domains = [];
  let citedSources = 0;
  let totalUrls = 0;
  ...
  for (const node of Object.values(raw.mapping)) {
    ...
    if (content?.content_type === 'code' && text) {
      try {
        ...
        if (matchSearch) { hiddenLikely = false; tryAddQuery(matchSearch[1], []); }
      }
    }
    ...
    refs.slice(0,500).forEach((ref)=>{
      ...
      ref.items.slice(0,500).forEach((item)=>{
        ...
        totalUrls += 1; citedSources += 1;
        citedUrlCounts.set(cleanUrl, (citedUrlCounts.get(cleanUrl) || 0) + 1);
        sourceItems.push({ url: cleanUrl, title: sanitizeString(item.title || item.display_text || item.name || item.citation_text || '', 300), domain:'', cited:true });
        if (cleanUrl.includes('utm_source=chatgpt')) utmCount += 1;
        try { const domain = normalizeDomain(new URL(cleanUrl).hostname); if (domain) domains.push(domain); } catch {}
      });
    });
  }
  const uniqueQueryKeys = new Set();
  const dedupedQueries = queries.filter((item) => { const key = `${item.q}||${item.domains.join(',')}`; if (uniqueQueryKeys.has(key)) return false; uniqueQueryKeys.add(key); return true; }).slice(0,250);
  ...
  sourceItems.forEach((item)=>{
    if (!item.url) return;
    const existing = sourceMap.get(item.url) || { url:item.url, title:item.title||'', domain:item.domain||'', count:0, citedCount:0 };
    existing.count += 1;
    if (!existing.title && item.title) existing.title = item.title;
    if (!existing.domain && item.domain) existing.domain = item.domain;
    existing.citedCount = citedUrlCounts.get(item.url) || existing.citedCount || 0;
    sourceMap.set(item.url, existing);
  });
  const sources = [...sourceMap.values()].map((item)=>{
    let domain = item.domain || '';
    if (!domain && item.url) { try { domain = normalizeDomain(new URL(item.url).hostname); } catch {} }
    const cited = (item.citedCount || 0) > 0;
    return { ...item, domain, status: cited ? 'cited' : 'considered', statusLabel: cited ? 'Cited source' : 'Considered source' };
  ...
  return {
    model: sanitizeString(raw.default_model_slug || '', 120) || null,
    ...
    hiddenLikely,
    searchOrigin: getSearchOrigin(summaryQueries.length, summaryCitedSources || summarySources.length, utmCount, summaryDomains.length),
    latestUserPrompt: latestTurn?.prompt || latestUserPrompt || '',
    conversationTurns: turnList,
    searchSignals: { explicitQueries: summaryQueries.length, contentRefs: summaryCitedSources, utmRefs: utmCount, externalDomains: summaryDomains.length },
    conversationId,
    pageUrl,
...
async function maybeCaptureTab(tabId, url) {
  try {
    if (isChatgptUrl(url)) {
      const res = await chrome.scripting.executeScript({ target:{tabId}, world:'MAIN', func: function fetchConversationPayloadInPage(){
        ...
        return (async()=>{
          try {
            ...
            const convoResp = await fetch('/backend-api/conversation/' + encodeURIComponent(cid), { credentials:'include', headers:{ Authorization:'Bearer ' + session.accessToken } });
          ...
          } catch (error) { return { error:`Page fetch failed: ${error?.message || 'Unknown error'}` }; }
        })();
      }});
